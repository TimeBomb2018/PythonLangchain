...
def summarize_messages(current_input):
    ...
    # 生成摘要（AIMessage）
    summary_ai_message = summarization_chain.invoke({'chat_history': messages_to_summarize})
    summary_message = SystemMessage(
        content=f"历史对话摘要: {summary_ai_message.content}",
...
result = final_chain.invoke({'input': '我的名字是什么？', 'config': {"configurable": {"session_id": "user123"}}},
